  ...
  return (
    ...
    >
      <p className="my-auto h-[90%] w-full overflow-x-hidden overflow-y-auto whitespace-pre-wrap">
        {task.content.length == 0 ? "New Task" : task.content}
      </p>
      {mouseIsOver && (
        <button
          onClick={() => {
          ...
          className="text-white absolute right-4 top-1/2 -translate-y-1/2 bg-columnBackgroundColor rounded p-2 opacity-60 hover:opacity-100"
        >
          <FaTrash />
        </button>
      )}
